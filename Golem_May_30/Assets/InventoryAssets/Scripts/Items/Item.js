	...
	playersinv.gameObject.SendMessage ("PlayPickUpSound", SendMessageOptions.DontRequireReceiver); //Play sound
		...
		if(stackable){
			...
			if(locatedit!=null){//if we have a stack to stack it to!
				getit=false;
				locatedit.stack+=1;
				Destroy(this.gameObject);					//Good idea? Bad idea?
			}
			else{
				getit=true;
			}
...
//Moves the item to the Players 'itemHolderObject' (inventory?) and disables it. In most cases this will just be the Inventory object.
function MoveMeToThePlayer(itemHolderObject : Transform){
	...
	if(gameObject.tag == "keepItemCollider"){ //For items that need colliders for effects etc
	...
	transform.localPosition = Vector3.zero;  //If you comment this out, you can drop item where it was picked up.
}
