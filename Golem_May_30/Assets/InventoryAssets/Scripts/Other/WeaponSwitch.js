...
function SelectWeapon (index : int){
	for (var i = 0; i < transform.childCount; i++){ //Loop through the weapons.
		...
		if (i == index){		
			...
			if(transform.GetChild(i).name == "magicAxeWeapon"){  //If weapon is called "magicAxeWeapon"
				//print("it's a magic axe!!");
								
				//transform.GetChild(i).renderer.material.SetColor ("_Color", Color.red);  //SET COLOR RED
				//transform.GetChild(i).Rotate(Vector3.right * 10);    //ROTATE OBJECT
			}                                        
			...
			transform.GetChild(i).gameObject.SetActive(true); //Activate the match
		}
		else{
